...
class EATMINTConfig:
    ...
    annotations_path: str = "ExternalAnnotations"

    # Sampling parameters
    physio_fs: int = 512  # Native physio sampling rate (Hz)
    annotations_fs: int = 100  # Annotations sampling rate (Hz)
    ...
    def annotations_dir(self) -> Path:
        return Path(self.data_root) / self.annotations_path
...
def load_arousal_annotations(
    ...
    """
    Load arousal annotations for a participant.
    
    Annotations are at 100Hz, covering first 10 minutes (600 seconds) of collaboration.
    
    Args:
        config: EATMINTConfig with data paths
        dyad: Dyad number
        participant: Participant number (1 or 2)
        cue_type: Which emotional cue annotations to use:
                  'sp' (speech only), 'hr' (heartbeat only), 'sphr' (speech+heartbeat)
    
    Returns:
        Dict with keys:
            'arousal': Averaged arousal across 6 annotators (shape: [n_samples])
            'arousal_raw': All 6 annotators separately (shape: [n_samples, 6])
            'fs': Sampling rate (100 Hz)
            'video_time_ms': VideoTime in milliseconds
    """
    annotations_file = (
        config.annotations_dir 
        / f"D{dyad:02d}_P{participant:02d}" 
        / f"D{dyad:02d}_P{participant:02d}_interpolated.csv"
    )
    ...
    # Get columns for specified cue type (e.g., sphr_A1, sphr_A2, ..., sphr_A6)
    annotator_cols = [f"{cue_type}_A{i}" for i in range(1, 7)]
    
    # Check if columns exist
    missing = [col for col in annotator_cols if col not in df.columns]
    if missing:
        raise ValueError(f"Missing annotation columns: {missing}")
    
    # Extract arousal values
    arousal_raw = df[annotator_cols].values.astype(np.float32)  # (n_samples, 6)
    arousal_mean = arousal_raw.mean(axis=1)  # (n_samples,)
    
    return {
        "arousal": arousal_mean,
        "arousal_raw": arousal_raw,
        "fs": config.annotations_fs,
        "video_time_ms": df["VideoTime"].values,
    }
...
class EATMINTPhysioDataset(Dataset):
    ...
    # Arousal targets (treated as additional modalities)
    # These are from external annotations - 3 cue types averaged across 6 annotators each
    AROUSAL_SIGNALS = ["arousal_sp", "arousal_hr", "arousal_sphr"]  # speech, heartbeat, both
    ...
    def __init__(
        ...
        arousal_cues: list[str] = None,  # Default: all 3 cues
        preload: bool = False,
    ):
        super().__init__()
        self.config = config
        ...
        self.arousal_cues = arousal_cues if arousal_cues is not None else ["sp", "hr", "sphr"]
    ...
    def _load_participant_data(
        ...
        # Load arousal annotations if requested
        if self.include_arousal:
            arousal_dict = {}
            for cue in self.arousal_cues:
                try:
                    arousal_data = load_arousal_annotations(
                        self.config, dyad, participant, cue_type=cue
                    )
                    # Arousal is at 100Hz, covers first 10 min (600 sec) of collaboration
                    # Normalize arousal values using robust IQR method
                    arousal = arousal_data["arousal"]
                    iqr = np.percentile(arousal, 75) - np.percentile(arousal, 25)
                    arousal_norm = (arousal - np.median(arousal)) / (iqr + 1e-6)
                    arousal_dict[f"arousal_{cue}"] = arousal_norm
                except Exception as e:
                    warnings.warn(f"Could not load arousal ({cue}) for D{dyad:02d}P{participant:02d}: {e}")
                    # Fill with zeros if annotations missing
                    arousal_len = int(600 * self.config.annotations_fs)  # 10 min at 100Hz
                    arousal_dict[f"arousal_{cue}"] = np.zeros(arousal_len, dtype=np.float32)
            
            # Store all arousal cues
            data["arousal"] = arousal_dict
            data["arousal_fs"] = self.config.annotations_fs
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Extract arousal windows if available and concatenate with physio
        if self.include_arousal and "arousal" in data:
            arousal_windows = []
            for cue in self.arousal_cues:
                arousal_window = self._extract_arousal_window(
                    data, window_info["start_sec"], cue_type=cue
                )
                arousal_windows.append(arousal_window[:, None])  # (seq_len, 1)
            
            # Concatenate all: (seq_len, 6+3) = (seq_len, 9)
            combined = np.concatenate([physio] + arousal_windows, axis=1)
            
            return {
                "physio": torch.from_numpy(combined).float(),
    ...
    def _extract_arousal_window(
        self, data: Dict[str, np.ndarray], start_sec: float, cue_type: str
    ) -> np.ndarray:
        """
        Extract arousal window matching physio window timespan for a specific cue.
        
        Arousal is at 100Hz, so we need to extract and downsample to target_fs.
        
        Args:
            data: Loaded participant data
            start_sec: Window start time in seconds
            cue_type: Which cue type ('sp', 'hr', 'sphr')
        
        Returns:
            Arousal array at target_fs: (seq_len,)
        """
        arousal_key = f"arousal_{cue_type}"
        arousal = data["arousal"][arousal_key]
        arousal_fs = data["arousal_fs"]
        
        # Calculate start/end samples in arousal space
        start_sample = int(start_sec * arousal_fs)
        arousal_window_samples = int(self.window_size_sec * arousal_fs)
        end_sample = start_sample + arousal_window_samples
        
        # Handle edge cases
        if start_sample >= len(arousal):
            # Beyond available data, return zeros
            target_samples = int(self.window_size_sec * self.target_fs)
            return np.zeros(target_samples, dtype=np.float32)
        
        end_sample = min(end_sample, len(arousal))
        arousal_segment = arousal[start_sample:end_sample]
        
        # Pad if needed
        if len(arousal_segment) < arousal_window_samples:
            pad_len = arousal_window_samples - len(arousal_segment)
            arousal_segment = np.pad(arousal_segment, (0, pad_len), mode='edge')
        
        # Resample to target_fs if needed
        if arousal_fs != self.target_fs:
            # Use polyphase resampling
            target_samples = int(self.window_size_sec * self.target_fs)
            arousal_resampled = signal.resample(arousal_segment, target_samples)
            return arousal_resampled.astype(np.float32)
        else:
            return arousal_segment.astype(np.float32)

    def get_signal_names(self) -> List[str]:
        """Return list of signal channel names (physio + arousal)."""
        names = list(self.PHYSIO_SIGNALS)
        if self.include_arousal:
            for cue in self.arousal_cues:
                names.append(f"arousal_{cue}")
        return names
...
def create_eatmint_dataset(
    ...
    """
    Convenience function to create an EATMINT physio dataset with arousal targets.

    Args:
        data_root: Path to EATMINT data root (containing Physio/, StartEndTimes/, ExternalAnnotations/)
        window_size_sec: Window duration in seconds
        hop_size_sec: Hop between windows in seconds
        ...
        include_arousal: Whether to load arousal annotations as additional input channels
        arousal_cues: List of cue types to load: 'sp', 'hr', 'sphr'. Default: all 3
        include_arousal: Whether to load arousal annotations as targets
    ...
    return EATMINTPhysioDataset(
        ...
        include_arousal=include_arousal,
        arousal_cues=arousal_cues,
        preload=preload,
    )
