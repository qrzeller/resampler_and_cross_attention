...
def parse_args() -> argparse.Namespace:
    ...
    # Patch-based tokenization parameters
    parser.add_argument(
        "--patch-len",
        type=int,
        default=50,
    ...
    parser.add_argument(
        "--use-conv-frontend",
        action="store_true",
        help="Use Conv1d stack for patch projection (better morphology handling)",
    )
    parser.add_argument(
        "--mask-strategy",
        type=str,
        default="mae",
        choices=["mae", "bert"],
        help="Masking strategy: 'mae' (remove masked tokens) or 'bert' (replace with [MASK])",
    )
    parser.add_argument(
        "--mask-type",
        type=str,
        default="channel_drop",
        choices=["random", "span", "channel_drop", "mixed"],
        help="Masking type: random, span, channel_drop (arousal only), or mixed (default: channel_drop)",
    )
    parser.add_argument(
        "--mask-ratio",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Probability of dropping each arousal channel (default: 0.5). Physio channels always visible.",
    )
    parser.add_argument(
        "--span-len-min",
        type=int,
        default=2,
        help="Minimum span length for span masking (default: 2 patches)",
    )
    parser.add_argument(
        "--span-len-max",
        type=int,
        default=8,
        help="Maximum span length for span masking (default: 8 patches)",
    )
...
def main() -> None:
    ...
    print(f"  Patch len: {args.patch_len}")
    print(f"  Conv frontend: {'enabled' if args.use_conv_frontend else 'disabled'}")
    ...
    # Prepare mask configuration for training
    mask_config = {
        'mask_type': args.mask_type,
        'mask_ratio': args.mask_ratio,
        'channel_drop_prob': args.channel_drop_prob,
        'span_len_min': args.span_len_min,
        'span_len_max': args.span_len_max,
        'physio_channels': 6,  # Number of physio channels (GSR, ECG, BVP_baseline, BVP_pulsatile, Resp, Temp)
    }
    
    if args.mask_type == 'channel_drop':
        print(f"  Channel drop: arousal channels only (physio always visible), drop_prob={args.channel_drop_prob:.2f}")
