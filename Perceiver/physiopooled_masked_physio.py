...
class PhysioMaskingConfig:
    ...
    channel_drop_prob: float = 0.6
    min_channels_to_drop: int = 1
...
def main() -> None:
    ...
    history = train_masked(
        ...
        lr=1e-4,
        device=device,
        save_path="physiopooled_masked_physio_checkpoint.pt",
    )

    plot_training_history(history, save_path="physiopooled_masked_history.png")
    visualize_reconstruction(
        model,
        dataset,
        sample_idx=0,
        device=device,
        save_path="physiopooled_masked_reconstruction.png",
    )
