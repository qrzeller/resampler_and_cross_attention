...
def train_epoch(
    ...
    for batch in tqdm(dataloader, desc="Train", leave=False):
        ...
        if modality_dropout_p > 0.0:
            ...
            b, _, c = physio.shape
            # channel_mask: 1 = keep channel, 0 = drop channel
            channel_mask = torch.bernoulli(
                torch.full((b, 1, c), keep_prob, device=physio.device)
            )
            # For input, zero out dropped channels (will be replaced with mask tokens in model)
            physio_in = physio * channel_mask
            # Create loss mask: 1 for dropped channels (where channel_mask=0), 0 for kept channels
            loss_mask = 1.0 - channel_mask
