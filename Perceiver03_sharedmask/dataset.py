...
class PhysioTimeSeriesDataset(Dataset):
    ...
    def __init__(
        ...
        mask_num_spans: int = 2,
        mask_shared_time: bool = True,
    ) -> None:
        """
        Args:
        ...
        self.mask_num_spans = max(1, int(mask_num_spans))
        self.mask_shared_time = bool(mask_shared_time)
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.apply_mask and self.mask_frac > 0.0:
            mask = self._create_mask(self.window_samples, self.num_signals)
            item["mask"] = mask
        return item

    def _create_mask(self, seq_len: int, channels: int) -> torch.Tensor:
        if self.mask_shared_time:
            return self._create_shared_time_span_mask(
                seq_len, channels, self.mask_frac, self.mask_num_spans
            )
        return self._create_channel_chunk_mask(seq_len, channels, self.mask_frac)

    def _create_shared_time_span_mask(
        self,
        seq_len: int,
        channels: int,
        mask_frac: float,
        ...
        """Create a boolean mask (seq_len, channels) with shared masked time spans.

        The same time indices are masked for all channels, using multiple
        contiguous spans whose total length is approximately mask_frac * seq_len.
        """
        seq_len = int(seq_len)
        channels = int(channels)
        num_spans = max(1, int(num_spans))
        total = max(1, int(round(seq_len * float(mask_frac))))

        base = total // num_spans
        rem = total - base * num_spans
        lengths = [base] * num_spans
        for i in range(rem):
            lengths[i] += 1
        ...
        for L in lengths:
            if L >= seq_len:
                time_mask[:] = True
                break
            start = np.random.randint(0, seq_len - L + 1)
            time_mask[start : start + L] = True

        mask = np.repeat(time_mask[:, None], channels, axis=1)
...
class PhysioResampledDataset(PhysioTimeSeriesDataset):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            mask_num_spans=mask_num_spans,
            mask_shared_time=mask_shared_time,
        )
