...
def parse_args() -> argparse.Namespace:
    ...
    preload_group.add_argument(
        ...
        dest="preload",
        action="store_true",
        help="Preload participant data and precompute all windows into RAM (default)",
    )
    preload_group.add_argument(
        "--no-preload",
        dest="preload",
        action="store_false",
        help="Disable preloading and window precompute",
    )
    parser.set_defaults(preload=True)
    parser.add_argument(
        "--cache-to-gpu",
        action="store_true",
        help="If windows are cached, transfer the window cache to GPU VRAM (may exhaust VRAM)",
    )
    ...
    # Masked modelling options
    parser.add_argument(
        "--masking",
        action="store_true",
        help="Enable masked modelling (dynamic, per-channel contiguous chunks)",
    )
    parser.add_argument(
        "--mask-frac",
        type=float,
        default=0.3,
        help="Approximate fraction of each channel's window to mask (default: 0.3)",
    )

    mask_mode = parser.add_mutually_exclusive_group()
    mask_mode.add_argument(
        "--mask-shared-time",
        action="store_true",
        help="Mask shared time spans across all channels (recommended)",
    )
    mask_mode.add_argument(
        "--mask-per-channel",
        action="store_true",
        help="Mask per-channel contiguous chunks (previous behavior)",
    )
    parser.add_argument(
        "--mask-num-spans",
        type=int,
        default=2,
        help="Number of masked time spans when using shared-time masking (default: 2)",
    )
...
def main() -> None:
    ...
    # Default masking protocol in this variant: shared-time spans
    if args.masking and (not args.mask_shared_time) and (not args.mask_per_channel):
        args.mask_shared_time = True
    ...
    if args.use_eatmint:
        ...
        dataset = create_eatmint_dataset(
            ...
            cache_windows=args.preload,
        )
        feature_names = dataset.get_signal_names()
        num_signals = len(feature_names)
        # If masking requested but dataset doesn't natively support it, wrap it
        if args.masking:
            import numpy as _np
            ...
            class MaskWrapper(_DS):
                ...
                def __getitem__(self, idx):
                    item = self.base[idx]
                    # item may be dict or tensor
                    if isinstance(item, dict):
                        phys = item.get("physio")
                    else:
                        phys = item
                        item = {"physio": phys}
                    ...
                    if args.mask_per_channel:
                        ...
                        total = max(1, int(round(seq_len * self.mask_frac)))
                        k = max(1, int(args.mask_num_spans))
                        base = total // k
                        rem = total - base * k
                        lengths = [base] * k
                        for i in range(rem):
                        ...
                        time_mask = _np.zeros((seq_len,), dtype=bool)
                        for L in lengths:
                            if L >= seq_len:
                                time_mask[:] = True
                                break
                            s = _np.random.randint(0, seq_len - L + 1)
                            time_mask[s : s + L] = True
                        mask = _np.repeat(time_mask[:, None], channels, axis=1)
                ...
                def window_samples(self):
                    return self.base.window_samples

            dataset = MaskWrapper(dataset, args.mask_frac)
        print(f"  Signals: {feature_names}")
        # Optionally move the cached windows to the GPU (if requested)
        if args.preload and args.cache_to_gpu:
            if device.type != "cuda":
                print("Warning: --cache-to-gpu requested but device is not CUDA. Skipping transfer.")
            else:
                try:
                    dataset.transfer_cache_to_device(device)
                except Exception as e:
                    print(f"Failed to transfer cache to device: {e}")
    else:
        # Use synthetic data
        ...
        dataset = PhysioResampledDataset(
            ...
            apply_mask=args.masking,
            mask_num_spans=args.mask_num_spans,
            mask_shared_time=(args.mask_shared_time and (not args.mask_per_channel)),
        )
    ...
    if args.masking and val_ds is not None:
        ...
        class FixedMaskWrapper(_DS):
            ...
            def _create_mask(self, phys, idx: int):
                seq_len = phys.shape[0]
                channels = phys.shape[1]

                rng = _np.random.default_rng(int(args.seed) + int(idx))
                ...
                if args.mask_per_channel:
                    ...
                    for ch in range(channels):
                        ...
                        if clen >= seq_len:
                            ...
                            s = int(rng.integers(0, seq_len - clen + 1))
                        mask[s : s + clen, ch] = True
                else:
                    total = max(1, int(round(seq_len * self.mask_frac)))
                    k = max(1, int(args.mask_num_spans))
                    ...
                    for L in lengths:
                        if L >= seq_len:
                            time_mask[:] = True
                            break
                        s = int(rng.integers(0, seq_len - L + 1))
                        time_mask[s : s + L] = True
                    mask = _np.repeat(time_mask[:, None], channels, axis=1)
            ...
            def __getitem__(self, idx):
                ...
                if idx not in self.cache:
                    if isinstance(item, dict) and "mask" in item:
                        ...
                        self.cache[idx] = self._create_mask(phys, idx)
    ...
    if args.preload and args.cache_to_gpu:
        if args.num_workers > 0:
            print(
                "Warning: cache_to_gpu enabled — forcing num_workers=0 to avoid duplicating GPU tensors in workers."
            )
        worker_count = 0
    ...
    train_loader = DataLoader(
        ...
        persistent_workers=True if worker_count > 0 else False,
    )
    val_loader = (
        DataLoader(
            val_ds,
