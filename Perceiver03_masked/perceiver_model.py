...
class PerceiverResampler(nn.Module):
    ...
    def __init__(
        ...
        # Learnable mask token (replaces masked signal values while keeping positional encodings)
        self.mask_token = nn.Parameter(torch.zeros(signal_dim, dtype=torch.float32))
    ...
    def forward(
        ...
        # If mask provided, replace masked positions in `series` with learnable mask token
        # mask expected shape: (batch, seq_len, signal_dim) or (batch, seq_len) or (seq_len, signal_dim)
        if mask is not None:
            if mask.dtype != torch.bool:
                mask = mask.bool()

            # Normalize mask shape to (batch, seq_len, signal_dim)
            if mask.dim() == 2:
                # (batch, seq_len) -> expand across channels
                mask = mask.unsqueeze(-1).expand(-1, -1, series.size(-1))
            elif mask.dim() == 2 and mask.size(0) == seq_len:
                # (seq_len, signal_dim) case -> expand batch
                mask = mask.unsqueeze(0).expand(batch, -1, -1)
            elif mask.dim() == 3 and mask.size(0) == seq_len and mask.size(1) == series.size(-1):
                # (seq_len, signal_dim) transpose case - unlikely, skip
                pass
            ...
            # Apply mask token
            mt = self.mask_token.view(1, 1, -1).to(series.device, dtype=series.dtype)
            series = series.clone()
            series[mask] = mt.expand_as(series)[mask]
